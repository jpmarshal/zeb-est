...
s.init <- zebData[[3]]
w.init <- zebData[[4]]
#my.inits <- function(){list(beta0 = runif(1, -1, 1),
#                            beta1 = runif(1, -1, 1),
#                            lsigma = runif(1, -1, 0.5),
#                            s = s.init, w = w.init)}

my.inits <- list(beta0 = runif(1, 4, 10),
                 beta1 = runif(1, 0, 5),
                 lsigma = runif(1, -1, 0.5),
                 s = s.init, w = w.init)
...
#            model.file = 'modelweibull.txt', parallel = T)
...
save(out, file = 'weibull150Out.RData')
